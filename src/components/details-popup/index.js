...
export default function DetailsPopup({ show, setShow }) {
  const {
    mediaDetails, setMediaDetails, similarMedias, setSimilarMedias, currentMediaInfoIdAndType, setCurrentMediaInfoIdAndType, loggedInAccount, key, setKey, router ,data:session } = useContext(GlobalContext);

  useEffect(() => {
    if (currentMediaInfoIdAndType !== null) {

      async function getMediaDetails() {
        const extractMediaDetails = await getTVorMovieDetailsByID(currentMediaInfoIdAndType.type, currentMediaInfoIdAndType.id);
